...
class JSValidator {
    ...
    public function password($f_equal = false, $min_len = true, $t_empty = false) {
        ...
        if($t_empty){
            $cl->t_empty = $this->message->get($t_empty);
        }else{
            //$this->message->get(ValidatePassword::CODE_EMPTY);
            $cl->t_empty = $this->message->get(ValidatePassword::CODE_EMPTY);
        }

        if($f_equel){
            $cl->f_equel = $f_equel
            $cl->t_equel = $this->message->get("ERROR_PASSWORD_CONF");
        }
    ...
    public function name($t_empty = false, $t_max_len = false, $t_type = false) {
        return $this->getBaseDate($t_empty, $t_max_len, $t_type, "ValidateName", "name");;
    }

    public function login($t_empty = false, $t_max_len = false, $t_type = false) {
        return $this->getBaseDate($t_empty, $t_max_len, $t_type, "ValidateLogin", "login");;
    }

    public function email($t_empty = false, $t_max_len = false, $t_type = false) {
        return $this->getBaseDate($t_empty, $t_max_len, $t_type, "ValidateEmail", "email");
    }
    ...
    private function getBaseDate($t_empty, $t_max_len, $t_type, $class, $type){
        ...
        if($t_max_len) {
            $cl->$t_max_len = $this->message->get($t_max_len);
        }else{
            $cl->$t_max_len = $this->message->get($class::CODE_MAX_LEN);
        }

        if($t_type) {
            $cl->t_type = $this->message->get($t_type);
        }else{
