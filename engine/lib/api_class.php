...
class API {
    ...
    public function addComment($parent_id, $article_id, $text){
        ...
        try {
            ...
            if($comment->isSave() && ($comment_parent->user_id != $this->auth_user->id)) {
                $user = new UserDB();
                $user->load($comment_parent->user_id);
                $this->mail->send($user->email, array("user" => $user, "link" => $comment_parent->link))
            }

            return json_encode(array("id" => $commnt->id, "parent_id" => $commnt->parent_id, "user_id" => $this->auth_user->id, "name" => $this->auth_user->name, "avatar" => $this->auth_user->avatar, "text" => $comment->text, "date" => $comment->date));
