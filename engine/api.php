...
}elseif ($request->func == "delete"){
    //+
    $result = $api->delete($request->obj, $request->id);
    //$result = $api->delete($request->obj, 2);
    //-
}elseif ($request->func == "add_comment"){
    $result = $api->addComment($request->$parent_id, $request->$article_id, $request->$text);
}
