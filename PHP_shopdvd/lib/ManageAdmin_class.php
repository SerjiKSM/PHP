...
class ManageAdmin extends Manage{
	...
	public function deleteProduct(){
		...
		if($this->product->delete($this->data["id"])){
			return $this->sm->message("SUCCESS_DELETE_PRODUCT");
		}else{
	...
	private function loadImage() {
		$img = $_FILES["img"];
		if(!$img["name"]){
			return $this->sm->message("ERROR_IMG");
		}
		if(!$this->isSecure($img)) {
			return false;
		}
		$uploadfile = "../".$this->config->dir_img_products.$img["name"];
		
		if(file_exists($uploadfile)){
			return $this->sm->message("ERROR_EXIST_IMG");
		}
	...
	private function isSecure($img) {
		...
		if(($type != "image/jpg") && ($type != "image/jpeg") && ($type != "image/png")) {
			return $this->sm->message("ERROR_TYPE_IMG");
		}
		if($size > $this->config->max_size_img){
			return $this->sm->message("ERROR_SIZE_IMG");
		}
		return true;
	}
	...
	public function deleteSection(){
		if($this->section->delete($this->data["id"])){
			return $this->sm->message("SUCCESS_DELETE_SECTION");
		}else{
			return $this->sm->unknownError();
		}
	}
	...
	public function addOrder() {
		...
		if($this->data["is_pay"] != 0) {
			$temp_data["date_pay"] = $this->format->ts();
		}else{
...
//		echo $date_pay;
		...
		//echo $this->data["is_pay"];
		//echo $this->data["is_send"];
		
		if(($this->data["is_pay"]) && ($date_pay == 0)){
			$temp_data["date_pay"] = $this->format->ts();
		}else{
			$temp_data["date_pay"] = $date_pay;
//			$temp_data["date_pay"] = "1111111111";
		}
	...
	public function deleteOrder() {
		if($this->order->delete($this->data["id"])){
			return $this->sm->message("SUCCESS_DELETE_ORDER");
		}else{
			return $this->sm->unknownError();
		}
	}
	...
	private function dataOrder() {
		...
		if($product_ids){
			$temp_data["product_ids"] = substr($product_ids, 0, -1);
		}
	...
	public function deleteDiscount() {
		if($this->discount->delete($this->data["id"])){
			return $this->sm->message("SUCCESS_DELETE_DISCOUNT");
		}else{
			return $this->sm->unknownError();
		}
	}
	...
	public function dataDiscount() {
		...
		if($this->discount->getOnCode($this->data["code"])){
			$this->sm->message("SUCCESS_DISCOUNT_EXISTS");
		}
		return $temp_data;
	}
