...
//		exit;
	...
	public function getAllOnSectionID($section_id, $sort, $up)
	{
		if (!$this->checkSortUp($sort, $up)) {
			return $this->transform($this->getAllOnField("section_id", $section_id));
		}
	...
	public function getProduct($id, $section_table) {
...
//		echo $query;
	...
	public function searchText($q, $sort, $up) {
		
		if (!$this->checkSortUp($sort, $up)) {
			return $this->transform(parent::search($q, array("title", "country", "year", "director", "cast", "description")));
		}
		
		return $this->transform(parent::search($q, array("title", "country", "year", "director", "cast", "description"), $sort, $up));
		
	}
	
	protected function checkData($data) {
		if(!$this->check->id($data["section_id"])){
		...
		if(!$this->check->year($data["year"])){
			return "ERROR_YEAR";
		}
		if(!$this->check->title($data["country"])){
			return "ERROR_COUNTRY";
		}
		if(!$this->check->title($data["director"])){
			return "ERROR_DIRECTOR";
		}
		if(!$this->check->play($data["play"])){
			return "ERROR_PLAY";
		}
		if(!$this->check->text($data["cast"])){
			return "ERROR_CAST";
		}
		if(!$this->check->text($data["description"])){
			return "ERROR_DESCRIPTION";
		}
		if(!$this->check->title($data["img"])){
			return "ERROR_IMG";
		}
		if(!$this->check->ts($data["date"])){
			return "UNKNOWN_ERROR";
		}
